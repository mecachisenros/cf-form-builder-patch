/**
 * Patched copy of caldera-forms/clients/form-builder/index.js
 * @see https://github.com/CalderaWP/Caldera-Forms/blob/master/clients/form-builder/index.js
 */
 ...
 * Import our patch.
 */
import { patchedPrepareProcessorsForSave } from "./patchPrepareProcesorsForSave"
...
const HandleSave = ({ jQuery, formId }) => {
	...
	const onSave = () => {
		...
		if (data_fields.config.processors) {
			...
			data_fields.config.processors = patchedPrepareProcessorsForSave(_processors, data_fields.config.processors); // use our patch to preserve the processors order
		} else {
